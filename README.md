...
**当前版本：v0.3.8 · [下载 XPI](https://github.com/DragonGong/zotero-math-patch/releases/latest/download/zotero-math-patch.xpi) · [更新记录](CHANGELOG.md)**
...
已有 Zotero `.math` 节点不会再与引用、附件等内容共用匿名占位符。它们会以 `[[READONLY_MATH:math-1]]` 这样的稳定标记以及精确 LaTeX 作为受保护上下文发送，帮助模型理解被拆开的混合公式。模型不应为标记生成独立行内操作；这类重复操作会被安全跳过。模型只有在合并一个被拆散的独立块级公式时才能选中含现有公式的文本块，并可根据数学含义规范化或修复该公式的 LaTeX。本地不再要求新旧 LaTeX 字符级相同，但仍拒绝把内部标记写入最终公式。引用、附件、链接、代码和 Zotero 特殊节点仍是不可修改的硬保护内容。
...
行内操作的 `occurrence` 是 `source` 在指定 `blockId` 内从 1 开始的出现序号，不跨段落累计。插件不会自动纠正 occurrence，也不会把操作搬到另一个 block；模型选择的位置必须精确成立，否则该项会进入模型修复流程。

初次返回仍然必须通过严格原文校验。若某一项失败且能够定位到具体 operation，插件会向模型发送聚焦后的相关文本块、失败项、相关候选项、精确匹配次数和校验错误，让模型自行选择 `replace` 或 `remove`；本地不猜测它原本想指向哪里。每个不同的失败操作最多尝试两次，每批最多尝试八次，其他候选操作保持不动。修正块级公式时，模型只选择连续的 `blockIds` 和 LaTeX；本地从模型选定的真实 DOM 块生成 canonical `source`。所有修正仍需重新通过块范围、受保护内容、重叠和 LaTeX 安全校验，失败时不会写回。

模型有时会同时返回一个完整块级公式和块内已包含的行内公式。v0.3.8 会在验证前归并这种可证明冗余的操作：只有当行内范围完全位于块级范围内，并且块级 LaTeX 已逐字包含行内 LaTeX 时，才跳过该行内项。这个过程不判断数学语义，也不修改模型的块级结果；无法证明冗余的重叠仍会进入修复流程或终止写回。
...
长笔记分批时会尽量携带相邻文本块作为上下文。完全相同的重复操作会去重；被完整块级结果逐字覆盖的行内操作会安全归并；针对同一原文的不同结果或无法证明冗余的重叠修改会终止整个写回。
...
4. 原生悬浮进度圆圈会显示准备、当前请求批次、校验、单项修复和保存等阶段；等待模型响应时会持续转动。
...
进度文字中的 `batch 2 of 5` 来自长笔记的真实分批状态。OpenAI Compatible 非流式请求不会提供单个请求内部的可靠 token 进度，因此插件用持续转动的圆圈表示当前请求仍在运行，不显示虚假的百分比或剩余时间。进入公式预览前圆圈会自动关闭；关闭预览时则持续显示到笔记保存完成。
...
- 菜单中没有 `Render Markdown Math` 或 `Process Math with AI`：确认插件已启用且版本为 v0.3.8，然后重启 Zotero。
...
- `Operation ... targets protected note content`：v0.3.6 起已把已有公式改为带精确 LaTeX 的受保护上下文，并会跳过模型对已有公式的重复行内操作。真正的引用、附件、链接或代码仍不会交给模型修改；若该错误持续出现，请通过七日追溯日志确认模型是否选择了这些硬保护节点。
- `Operation ... must preserve every existing read-only formula exactly`：这是 v0.3.6 的字符级限制。v0.3.7 已移除；模型在重建破碎块公式时可以将 `|` 规范化为 `\|`、调整括号或修复旧 LaTeX。
- `Operation ... overlaps another model operation`：v0.3.8 会自动归并被完整块级结果覆盖的重复行内操作，并在提示词中明确禁止模型生成这类重复项。不同结果之间的真实重叠仍会被拒绝，避免插件自行猜测应保留哪一个公式。

## 数据与隐私

AI 模式会发送普通文本块及其 `id`、`tag` 和 `text`。为帮助识别跨段公式，长笔记批次可能包含少量相邻普通文本上下文。已有 `.math` 节点会以稳定标记、公式类型和不含 `$`/`$$` 分隔符的精确 LaTeX 发送，供模型理解上下文，但不能作为独立行内修改目标；在重建一个包含普通破碎文本的块级公式时，模型可以根据数学含义吸收、规范化或修复这些现有公式片段。若初次操作未通过本地校验，插件还会向同一模型服务发送聚焦后的相关安全文本块、失败操作、相关候选操作、结构化匹配诊断和校验错误。每个失败操作最多修正两次、每批最多八次；不会为修正请求发送整篇 HTML。
...
这些硬保护位置在发送给模型的相邻文本中只表示为受保护占位符。插件不会执行模型返回的 HTML 或脚本。已有 `.math` 公式会作为受保护 LaTeX 上下文发送，模型对它们的重复行内操作会被忽略；含已有公式的块级重建由模型判断数学含义，本地只验证选定块、硬保护边界、操作重叠、LaTeX 安全和原子写入。
...
chrome/content/ai-progress.js      Zotero 原生 AI 进度圆圈与状态文字
