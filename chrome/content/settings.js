...
12. latex 必须语法完整，所有未转义的 { 和 } 必须成对闭合。
...
inline 操作必须包含 type、blockId、source、occurrence、latex。必须从实际包含 source 的同一个输入块对象中原样复制 id 作为 blockId，返回前确认 source 是该块 text 的完全一致子串；不要推算、递增或重编号 blockId。occurrence 是 source 在指定 blockId 内从 1 开始的序号，不能跨 block 计数；该 block 内只有一次时必须返回 1。block 操作必须包含 type、blockIds、source、latex。block 的 source 必须按 blockIds 顺序原样拼接对应 text，并以 U+000A 连接，不得改写原文。latex 不包含 $、$$ 或 HTML，所有未转义的大括号必须成对闭合。`;
  ...
  function normalizeSetting(name, value) {
    ...
    if (name === "showPreview" || name === "loggingEnabled") {
